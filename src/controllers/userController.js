...
export const decStrength = async(user)=>{
  ...
  const strangeDec = user.cow.strength - diffHour*config.decStrengthVal;
  const newStrength =  strangeDec >= 0 ? Math.round(strangeDec*100)/100:0;
  await UserModel.findByIdAndUpdate(user._id,{$set:{"cow.strength":newStrength}} , {new: true});
  return newStrength;
}

export const incStrength = async(user)=>{
  const randNew = Math.round((user.cow.strength + (Math.random()*2+4))*100)/100;
  const newStrength = randNew <= 100 ? randNew:100;
  const updatedCow ={...user.cow, strength: newStrength,lastFeedingTime: new Date()};
  await UserModel.findByIdAndUpdate(user._id,{$set:{"cow.strength":updatedCow.strength, "cow.lastFeedingTime": new Date()}},{new: true});
  return newStrength;
} 