	...
	if(user){
		...
		milkTank.forEach(element => {
			if(new Date(element.takingTime).getDay() == new Date(date).getDay() &&
    			...
    			new Date(element.takingTime).getFullYear() == new Date(date).getFullYear()) total+=element.milk;
		});
	}
	return Math.round(total*100)/100;
};
...
export const decStrength = async user=>{
	const diffTime = new Date() - user.cow.lastFeedingTime,
		...
		newStrength =  strangeDec >= 0
			? Math.round(strangeDec*100)/100
			:0;
...
export const incStrength = async user=>{
	const randNew = Math.round((user.cow.strength + (Math.random()*2+4))*100)/100,
		newStrength = randNew <= 100
			? randNew
			:100,
		updatedCow ={...user.cow,
			strength: newStrength,
			lastFeedingTime: new Date()};
	await UserModel.findByIdAndUpdate(user._id,{$set: {'cow.strength': updatedCow.strength,
		'cow.lastFeedingTime': new Date()}},{new: true});
	return newStrength;
}; 
