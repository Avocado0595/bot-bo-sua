...
import {decStrength, getUser, getTotalMilkByDay, updateUser, addUser} from '../controllers/userController.js';

const milk = async (message)=>{
        const user = await getUser(message.author.id);
        const milk = Math.round((Math.random()*(config.maxMilk-config.minMilk)+config.minMilk)*100)/100;
        if(user){
            const newStrength = await decStrength(user);
            const diffTime = user.lastTimeTakeMilk - new Date();
            const diffSecond = Math.abs(Math.ceil(diffTime / (1000)));
            const timeLeft = config.coolDownTime-diffSecond;
            
            if(timeLeft>0){
                const timeLeftMin = Math.round(timeLeft/60);
                const timeLeftSec = timeLeft%60;
                message.reply(`**${message.author.tag.split("#")[0]}** vắt từ từ thôi nè ! Chờ ${timeLeftMin} phút ${timeLeftSec} giây nữa vắt tiếp nhé :"> !`);  
            }
            else {
                if(newStrength <50)
                    message.reply("Bò đang đói, hãy cho bò ăn để có sữa nhé!\nDùng ```b!anco``` để cho bò ăn nè");
            else{
            ...
            const newUser = {cow:{strength: 100,
                dateOfBirth:  new Date(),
                lastFeedingTime: new Date()},
                userId: message.author.id, lastTimeTakeMilk: new Date(), 
                milkTank: [{milk:milk, takingTime: new Date() }], 
                totalMilk: milk};
