...
const milk = async message=>{
	const user = await getUser(message.author.id),
		milk = Math.round((Math.random()*(config.maxMilk-config.minMilk)+config.minMilk)*100)/100;
	if(user){
		...
		if(timeLeft>0){
		...
		} else if(newStrength <50) message.reply('Bò đang đói, hãy cho bò ăn để có sữa nhé!\nDùng ```b!anco``` để cho bò ăn nè');
		else{
			...
			if(totalMilk <= config.maxMilkPerDay){
				...
				const editUser = {totalMilk: user.totalMilk + milk,
